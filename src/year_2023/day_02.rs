...
impl Solver for Solution {
    ...
    fn with_input(&mut self, input: &str) {
        self.input = input
            ...
            .map(|l| {
                ...
                let game = s[0].replace("Game ", "");
                let hands = s[1]
                    .split(";")
                    .map(|h| {
                        h.split(",")
                            .map(|c| {
                                let cc = c.trim().split(" ").collect::<Vec<&str>>();
                                let count = cc[0];
                                let colour = cc[1];
                                (count.parse::<usize>().unwrap(), colour)
                            })
                            .fold(Hand::default(), |mut acc, c| {
                                match c.1 {
                                    "blue" => acc.blue = c.0,
                                    "red" => acc.red = c.0,
                                    "green" => acc.green = c.0,
    ...
    fn solve_part2(&self) -> String {
        self.input
            .iter()
            .map(|g| {
                let hand = g.hands.iter().fold(Hand::default(), |mut acc, h| {
                    acc.blue = acc.blue.max(h.blue);
                    acc.red = acc.red.max(h.red);
                    acc.green = acc.green.max(h.green);
                    acc
                });
